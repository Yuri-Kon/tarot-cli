 ...
 * 一个牌阵通常由以下要素构成：
 * <ul>
 *     <li>一个便于展示与记录的<strong>名称</strong>；</li>
 *     <li>一组有固定语义的<strong>牌位</strong>（如「过去 / 现在 / 未来」等），顺序即为抽牌顺序；</li>
 *     <li>与牌位数量相对应的<strong>需抽取牌数</strong>。</li>
 * </ul>
 * 实现类只需要定义名称与牌位列表，默认的 {@link #getCardCount()} 会直接使用牌位数量。
 *
 * <p><strong>线程安全性</strong>：接口本身不对线程安全做任何保证，具体实现可选择不可变实现以便在多线程环境重用。</p>
 *
 * @author  Tarot
 * @since   1.0
 */
public interface Spread {
     ...
     * 该名称通常用于：
     * <ul>
     *     <li>命令行或 UI 展示（例如「三张牌——过去 / 现在 / 未来」）；</li>
     *     <li>日志或历史记录中的标识；</li>
     *     <li>对用户说明当前使用的是哪一种牌阵。</li>
     * </ul>
     *
     ...
     * <ul>
     *     <li>实现类应返回一个按顺序排好、长度固定的列表；</li>
     *     <li>推荐返回不可变列表，避免调用方在运行时修改其内容；</li>
     *     <li>当列表为空时，表示该牌阵不需要抽牌（一般不推荐）。</li>
     * </ul>
     *
     ...
     * 返回该牌阵需要抽取的牌数。
     * <p>
     * 默认实现直接使用 {@link #getPositions()} 的列表长度，
     * 即「每个牌位正好对应一张要抽取的牌」。

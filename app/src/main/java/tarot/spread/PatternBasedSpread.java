 ...
 *
 * <p>
 * 通过组合 {@link SpreadPattern}，可以支持任意张数的牌阵， 而无需为 3 张、4 张、N 张分别写不同的 {@link Spread}
 * 实现类。
 *
 * <h2>命名策略</h2>
 *
 * <ul>
 * <li>如果 {@code namePrefix} 为非空且非空白，则名称为 「{@code namePrefix} + '：' +
 * pattern.getDisplayName()」。
 * <li>否则直接返回 {@code pattern.getDisplayName()}。
 * </ul>
 *
 * <h2>规约</h2>
 *
 * <ul>
 * <li>{@link #getPositions()} 直接委托给 {@link SpreadPattern#getPositions()}。
 * <li>调用方不应修改返回的列表；实现类不保证其可变性。
 * </ul>
 */
